  ...
  <div>
    <Form :label-width="90" inline @keydown.native.enter.prevent="getDataList('search')">
      ...
      <FormItem :label="item.label" v-for="(item, index) in searchInputNumberType" :key="index">
        <InputNumber
          ...
          :min="0"
          :max="search[item.max] ? search[item.max] : Infinity"
        ></InputNumber>
        —
        ...
        <Button
          ...
          size="small"
          title="清空该项输入"
          @click="clearInputNumber(item.max, item.min)"
        ></Button>
      </FormItem>
      <FormItem label="排序">
      ...
      <div>
        ...
        >
          <FormItem label="品名" prop="name">
            <Input v-model="modalParams.name" placeholder="必填，长度 100 以内" style="width: 250px"></Input>
          </FormItem>
          <FormItem label="标准进价" prop="standard_buy_unit_price">
            <Input
              v-model.number="modalParams.standard_buy_unit_price"
          ...
          <FormItem label="标准售价" prop="standard_sell_unit_price">
            <Input
              v-model.number="modalParams.standard_sell_unit_price"
              placeholder="非必填，小数位不超过2位的正整数"
              style="width: 250px"
            ></Input>
          </FormItem>
    ...
    <Modal v-model="delModalShow" width="370">
      ...
      <div style="text-align:center;line-height: 26px;">
        <p>
          将永久删除 <strong>{{ modalParams.name }}</strong> ，并删除该物品所有进出明细，且<strong>无法恢复</strong>。
        </p>
        <p>请输入该物品品名以确认删除。</p>
...
export default {
  ...
  methods: {
    ...
    getDataList(method) {
      ...
      searchParams.buyPriceMin !== null
        ? (whereSQL += `AND standard_buy_unit_price >= ${searchParams.buyPriceMin} `)
        : null
      searchParams.buyPriceMax !== null
        ? (whereSQL += `AND standard_buy_unit_price <= ${searchParams.buyPriceMax} `)
        : null
      searchParams.sellPriceMin !== null
        ? (whereSQL += `AND standard_sell_unit_price >= ${searchParams.sellPriceMin} `)
        : null
      searchParams.sellPriceMax !== null
        ? (whereSQL += `AND standard_sell_unit_price <= ${searchParams.sellPriceMax} `)
        : null
    ...
    addConfirm() {
      this.$refs.formVali.validate(valid => {
        if (valid) {
          ...
          VALUES ('${modalParams.name}','0','0','${modalParams.standard_buy_unit_price}','${
                  modalParams.standard_sell_unit_price
                }','${modalParams.remark}','${Date.now()}','')`
                this.$logger(SQL)
                this.$db.run(SQL, err => {
                  if (err) {
                    this.$logger(err)
    ...
    downloadExcel() {
      ...
      this.$db.all(this.downloadExcelSQL, (err, res) => {
        if (err) {
          ...
          const data = [['品名', '数量', '标准进价', '标准售价', '总金额', '备注', '创建时间', '修改时间']]
